...
function Avaliacao() {
  ...
  const buildMessage = () => {
    const lines = [
      ...
      ``,
      `*Procedimento(s) de interesse:* ${form.procedimentos.join(", ") || "—"}`,
      `*Objetivo/queixa principal:* ${form.objetivo || "—"}`,
      ``,
      `*Cirurgias prévias:* ${form.temCirurgia ? form.cirurgias || "sim, sem detalhe" : "não"}`,
      `*Condições de saúde:* ${form.condicoes.join(", ") || "nenhuma informada"}`,
      ...
      ``,
      `*Medicamento em uso:* ${form.usaMedicamento ? form.medicamentos || "sim, sem detalhe" : "não"}`,
      ...
      `*Exposição solar recente:* ${form.exposicaoSolar === true ? "sim" : form.exposicaoSolar === false ? "não" : "—"}`,
      ``,
